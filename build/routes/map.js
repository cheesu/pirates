...
router.get('/nextMap/:mapName', function (req, res) {
    ...
    _map2.default.findOne({ mapName: req.params.mapName }, function (err, map) {
        ...
        _map2.default.findOne({ mapName: thisMap.next }, function (err, nextMap) {
            if (err) throw err;
            // RETURN SUCCESS
            _monster2.default.findOne({ mapName: nextMap.mapName, type: 'boss' }, function (err, mon) {
                if (err) throw err;
                var monster = eval(mon);
                if (monster == null) {
                    return res.json({
...
router.get('/prevMap/:mapName', function (req, res) {
    ...
    _map2.default.findOne({ mapName: req.params.mapName }, function (err, map) {
        ...
        _map2.default.findOne({ mapName: thisMap.prev }, function (err, prevMap) {
            if (err) throw err;
            // RETURN SUCCESS
            _monster2.default.findOne({ mapName: prevMap.mapName, type: 'boss' }, function (err, mon) {
                if (err) throw err;
                var monster = eval(mon);
                if (monster == null) {
                    return res.json({
