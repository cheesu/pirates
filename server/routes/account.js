...
router.get('/extendSocket/:lv', (req, res) => {
        ...
        if(userInfo.mount.w.type != 'private'){
          ...
          Account.update({username: userInfo.username},{$set:{itemCount:userInfo.itemCount, gold:userInfo.gold}}, function(err, output){
            ...
            if(socketLV==1){
              ...
              let socket2 = {test:1};
              Item.update({id:userInfo.mount.w.id},{$set:{socket2:socket2}},function(err, output){
                let resultMsg = req.session.loginInfo.username+"님의 ["+userInfo.mount.w.name+"]이(가) 2단계 소켓 확장에 성공 하였습니다.";
                res.json({msg:resultMsg, result:true});
              });
            }else if(socketLV==3){
              let socket3 = {test:1};
              Item.update({id:userInfo.mount.w.id},{$set:{socket3:socket3}},function(err, output){
                let resultMsg = req.session.loginInfo.username+"님의 ["+userInfo.mount.w.name+"]이(가) 3단계 소켓 확장에 성공 하였습니다.";
                res.json({msg:resultMsg, result:true});
              });
            }
              ...
              .exec((err, item) => {
                ...
                Account.update({username: userInfo.username},{$set:{itemCount:userInfo.itemCount}}, function(err, output){
                  if(itemInfo.kind == "socket1"){
                    ...
                    Item.update({id: userInfo.mount.w.id},{$set:{socket2:itemInfo}}, function(err, output){
                      let resultMsg = req.session.loginInfo.username+"님의 ["+userInfo.mount.w.name+"]이(가) 소켓석 장착에 성공 하였습니다";
                      res.json({msg:resultMsg, result:true});
                    });
                  }
...
// 노예 경험치 구매
router.post('/buySlaveExp/', (req, res) => {
  ...
  Account.find({username: req.session.loginInfo.username})
      .exec((err, accounts) => {
          ...
          Slave.find({master: userInfo.username})
              .exec((err, slv) => {
                  if(err) throw err;
                  let slvInfo = eval(slv[0]);
                  ...
                  let userGold = userInfo.gold - gold;
                  Account.update({username: req.session.loginInfo.username},{$set:{gold:userGold}}, function(err, output){
                      if(err) throw err;

                      let slaveExp = slvInfo.exp + gold;
                              // 기존노예에 새로운 노예로 업데이트
                              Slave.update({master: req.session.loginInfo.username},{$set:{exp: slaveExp}}, function(err, output){
                                if(err) throw err;
                                let resultMsg = req.session.loginInfo.username+"님의 노예 ["+slvInfo.name+"]의 경험치를 "+gold+" 구매 하였습니다.";
                                res.json({msg:resultMsg, result:true});
                              });
                  });
                }
